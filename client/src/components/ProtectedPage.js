...
function ProtectedPage({ children }) {
  ...
  // Wrap getUser in useCallback to avoid unnecessary re-renders
  ...
  // Wrap getNotifications in useCallback to avoid unnecessary re-renders
  ...
  useEffect(() => {
    if (localStorage.getItem("token")) {
      getUser(); // Call getUser if there's a token
    } else {
      navigate("/login");
    }
  }, [getUser, navigate]); // Add getUser to dependency array to satisfy lint rule

  useEffect(() => {
    if (user) {
      getNotifications(); // Fetch notifications once the user is logged in
    }
  }, [user, getNotifications]); // Add getNotifications to the dependency array
