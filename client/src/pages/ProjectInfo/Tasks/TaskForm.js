import { Button, Form, Input, message, Modal, Tabs, Upload, DatePicker } from "antd";
...
function TaskForm({
  ...
  const validateEmail = () => {
    ...
    return employeesInProject.some(
        ...
        <Tabs.TabPane tab="Task Details" key="1">
          <Form
            ...
            initialValues={{
              ...
              deadline: task ? moment(task.deadline) : null, // Format date using moment
            }}
          >
            ...
            <Form.Item label="Task Description" name="description" required>
              <TextArea />
            </Form.Item>

            <Form.Item label="Assign To" name="assignedTo" required>
              <Input
          ...
          <div className="flex gap-5 mb-5">
            {images.map((image) => (
              <div key={image} className="flex gap-3 p-2 border border-solid rounded border-gray-500 items-end">
                <img src={image} alt="" className="w-20 h-20 object-cover mt-2" />
